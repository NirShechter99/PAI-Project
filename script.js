...
const levels = [
    ...
    {
        id: 3,
        instruction: "Move the defenders to the bottom line of the pitch to defend the goal.",
        players: 4,
        expected: { display: "flex", "align-items": "flex-end" }
    },
    {
        id: 4,
        instruction: "We have too many midfielders for one line! Allow them to wrap into multiple rows.",
        players: 18,
...
function updatePitchStyle() {
    selects.forEach(select => {
        ...
        // Apply directly to the inline style of the pitch board
...
function attachEventListeners() {
    // Live update when user changes dropdowns
    selects.forEach(select => {
...
function resetProperties() {
    // Set dropdowns back to first default options
    selects.forEach(select => {
        select.selectedIndex = 0;
    });
    
    // Clear inline styles to reset board
    pitch.style = "";
    
    // Special case: we need 'display' to default to block visually as a base constraint
...
function checkSolution() {
    ...
    // Check if current select values match expected level configuration
